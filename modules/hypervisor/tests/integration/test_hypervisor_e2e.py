...
class TestFullLifecycle:
    ...
    async def test_complete_session_lifecycle(self):
        ...
        # Terminate — should get merkle root
    ...
    async def test_session_without_audit(self):
        """Session with audit disabled returns None merkle root."""
        session = await self.hv.create_session(
...
class TestVouchingSlashingIntegration:
    ...
    def test_slash_cascades_to_voucher(self):
        """Slashing a vouchee also clips the voucher's collateral."""
        self.hv.vouching.vouch(
            "did:mesh:high", "did:mesh:low", self.session_id, 0.9, bond_pct=0.3
        )
        ...
        assert agent_scores["did:mesh:low"] == 0.0  # blacklisted
        assert agent_scores["did:mesh:high"] < 0.9  # clipped
...
class TestAuditTrailIntegration:
    ...
    async def test_merkle_chain_integrity(self):
        """Delta chain maintains cryptographic integrity."""
        session = await self.hv.create_session(
            config=SessionConfig(), creator_did="did:mesh:admin"
        )
        for i in range(10):
    ...
    async def test_merkle_root_deterministic(self):
        """Same session with same deltas produces consistent merkle roots."""
        session = await self.hv.create_session(
            config=SessionConfig(), creator_did="did:mesh:admin"
        )
        ...
        root2 = session.delta_engine.compute_merkle_root()
