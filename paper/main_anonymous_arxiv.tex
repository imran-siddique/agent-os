...
Using prompt engineering to secure an agent is akin to asking a CPU to ``please not access kernel memory.'' It is an architectural category error. To build reliable agentic systems, we must move from \textit{prompt engineering} to \textit{systems engineering}. For complementary research addressing reasoning failures through iterative refinement rather than hard constraints, see our concurrent preprint on runtime policy adaptation~\cite{anonymous2026companion}.
...
Several systems attempt to add safety layers to LLM applications. \textbf{Guardrails AI}~\cite{guardrailsai2023} provides output validation through schema enforcement and semantic checks. \textbf{NeMo Guardrails}~\cite{nvidia2023nemo} uses dialog management to steer conversations away from harmful topics. \textbf{Llama Guard}~\cite{inan2023llamaguard} fine-tunes a classifier to detect unsafe content. However, all three operate at the \textit{content level}---they filter what agents say, not what they do. An agent could generate safe-sounding text while executing dangerous actions.

\subsection{Agent Frameworks}
Modern agent frameworks like \textbf{LangChain}~\cite{langchain2022}, \textbf{AutoGPT}~\cite{autogpt2023}, \textbf{AutoGen}~\cite{autogen2023}, and \textbf{CrewAI}~\cite{crewai2024} provide powerful orchestration capabilities but delegate safety to the underlying LLM or optional middleware. None implement deterministic action-level governance as a first-class primitive.

\subsection{Agent Safety Evaluation}
Recent benchmarks have emerged for evaluating agent safety. \textbf{MAESTRO}~\cite{maestro2025} provides a multi-agent evaluation framework focusing on coordination failures. \textbf{WildGuard}~\cite{wildguard2024} evaluates guardrails against in-the-wild adversarial inputs. The \textbf{Agent Safety Framework}~\cite{safety2024framework} proposes taxonomies for agent risks. Our work differs by providing a \textit{solution architecture} rather than an evaluation framework, though we adopt similar adversarial testing methodologies.

\subsection{Access Control Models}
Attribute-Based Access Control (ABAC)~\cite{nist2014abac} has been the standard for fine-grained authorization in enterprise systems. We extend ABAC to agent contexts by treating agent identity, action type, target resource, and temporal context as attributes evaluated against deterministic policies.
...
For a complementary approach addressing reasoning failures through runtime policy adaptation rather than hard constraints, see our concurrent work~\cite{anonymous2026companion}.
