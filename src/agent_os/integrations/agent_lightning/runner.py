...
class GovernedRunner(Generic[T_task]):
    ...
    def __init__(
        ...
        *,
        ...
        logger.info(f"GovernedRunner initialized with kernel policies")
    ...
    async def iter(self, *, event: Optional[Any] = None) -> None:
        """
    ...
    async def _get_next_task(self) -> Optional[T_task]:
        ...
        try:
            ...
            if rollout.violations:
                emit_annotation({
                    ...
                    "agent_os.policies_violated": list(set(v.policy_name for v in rollout.violations)),
                })
